...
void main() {
  group('exportBundle', () {
    ...
    test(
      'respects the period window and matches the screen aggregates',
      () async {
        ...
        for (var i = 1; i < week.sales.length; i++) {
          expect(
            week.sales[i - 1].soldAt.isBefore(week.sales[i].soldAt),
            isFalse,
          );
        }
      },
    ...
    test('the sales query is period-filtered in SQL, not in memory', () async {
      final interceptor = _RecordingInterceptor();
      final loggedDb = AppDatabase(
      ...
      expect(
        salesSelects.every((s) => s.contains('WHERE') && s.contains('sold_at')),
        isTrue,
        reason:
            'week export must filter sales in the database: '
            '$salesSelects',
      );
    ...
    test(
      'DriftStore and MemoryStore build identical bundles (parity)',
      () async {
  ...
  group('CSV export', () {
    ...
    test('discounted, legacy, and normal sales export faithfully', () async {
      ...
      final salesTotalRow = rows.firstWhere(
        (r) => r.isNotEmpty && r[0] == 'Sales total (₦)',
      );
      expect(salesTotalRow[1], bundle.report.salesTotal);
    });

    test(
      'collected credit sales are marked in the credit_collected column',
      () async {
        ...
        final credit = (await memory.watchOwedCredits().first).firstWhere(
          (c) => c.customerName == 'Ngozi',
        );
        await memory.markCreditPaid(credit.saleId);

        final csv = buildReportCsv(
          await memory.exportBundle(ReportPeriod.week),
        );
        final rows = const CsvToListConverter(eol: '\n').convert(csv);
        final header = rows.firstWhere((r) => r.isNotEmpty && r[0] == 'date');
        int col(String name) => header.indexOf(name);

        final collected = rows.firstWhere(
          (r) => r.length == header.length && r[col('customer')] == 'Ngozi',
        );
  ...
  group('PDF export', () {
    test('builds a real PDF with the bundled Inter fonts', () async {
      ...
      ByteData ttf(String file) =>
          ByteData.view(File('assets/fonts/$file').readAsBytesSync().buffer);
