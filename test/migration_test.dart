...
void main() {
  test(
    'v2 → v3 migration adds sales.unit_cost and keeps old rows readable',
    () async {
      final dir = await Directory.systemTemp.createTemp(
        'prosperflow_migration',
      );
    ...
    ''');
      ...
      final legacy = await (db.select(
        db.sales,
      )..where((s) => s.id.equals('s-legacy'))).getSingle();
      ...
      final rows = await (db.select(
        db.sales,
      )..where((s) => s.id.equals('s-legacy').not())).get();
      expect(rows.single.unitCost, 6800);
    },
  );

  test(
    'v3 → v4 migration adds sales.list_price and keeps old rows readable',
    () async {
    ...
    ''');
      ...
      final legacy = await (db.select(
        db.sales,
      )..where((s) => s.id.equals('s-v3'))).getSingle();
      ...
      final discounted = await (db.select(
        db.sales,
      )..where((s) => s.id.equals('s-v3').not())).get();
