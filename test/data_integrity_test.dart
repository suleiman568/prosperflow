...
void main() {
  group('DriftStore', () {
    ...
    test(
      'overselling clamps stock at zero instead of going negative',
      () async {
        final water = (await store.watchProducts().first).firstWhere(
          (p) => p.name == 'Bottled Water (500ml)',
        );
    ...
    test(
      'concurrent syncNow calls apply each outbox row exactly once',
      () async {
        final backend = _RecordingBackend();
        final connectivity = StreamController<bool>.broadcast();
        final engine = DriftSyncEngine(
        ...
        final saleCreates = backend.applied.where(
