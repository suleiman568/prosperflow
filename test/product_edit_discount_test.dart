...
void main() {
  group('DriftStore', () {
    ...
    test(
      'updateProduct edits fields, keeps stock, and fills the outbox',
      () async {
        ...
        final after = (await store.watchProducts().first).firstWhere(
          (p) => p.id == before.id,
        );
    ...
    test('a rename relabels past sales in history (read-time names)', () async {
      ...
      final group = history.groups.firstWhere(
        (g) => g.productId == palmBefore.id,
      );
      expect(group.productName, 'Red Oil');
    });

    test(
      'editing a product with pending offline sales stays consistent',
      () async {
        ...
        final salePush = backend.applied.firstWhere(
          (r) => r.$1 == 'sale' && r.$2 == 'create',
        );
        expect(salePush.$3['unit_price'], palm.sellPrice);
        final productEdits = backend.applied.where(
          (r) => r.$1 == 'product' && r.$3['sell_price'] == 9900,
        );
        expect(productEdits, hasLength(1));
        expect(
          backend.applied.indexOf(salePush),
    ...
    test(
      'recordSale with a custom price stores unitPrice + listPrice',
      () async {
        final palm = await palmRow();
        await store.recordSale(
          productId: palm.id,
        ...
        final salePush = (await db.select(db.outbox).get()).firstWhere(
          (o) => o.entity == 'sale',
        );
    ...
    test(
      'updateProduct with a nonexistent id is a silent no-op (parity)',
      () async {
        ...
        final driftNames = (await store.watchProducts().first)
            .map((p) => p.name)
            .toSet();
        final memoryNames = (await memory.watchProducts().first)
            .map((p) => p.name)
            .toSet();
    ...
    test(
      'DriftStore and MemoryStore agree after the same edit + discount',
      () async {
        final seed = SeedData.build(DateTime.now());
        ...
        final driftPalm = (await store.watchProducts().first).firstWhere(
          (p) => p.id == palmId,
        );
        final memoryPalm = (await memory.watchProducts().first).firstWhere(
          (p) => p.id == palmId,
        );
        ...
        final driftGroup = driftHistory.groups.firstWhere(
          (g) => g.productId == palmId,
        );
        final memoryGroup = memoryHistory.groups.firstWhere(
          (g) => g.productId == palmId,
        );
        ...
        final driftEntry = driftGroup.entries.firstWhere(
          (e) => e.unitPrice == 9000,
        );
        final memoryEntry = memoryGroup.entries.firstWhere(
          (e) => e.unitPrice == 9000,
        );
  ...
  group('Products UI', () {
    ...
    testWidgets(
      'a failing save keeps the edit sheet open with an error toast',
      (tester) async {
        usePhoneSurface(tester);
  ...
  group('Record Sale UI', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.textContaining('Below cost — this sale loses'),
          findsOneWidget,
        );
  ...
  group('Reports UI', () {
    testWidgets('discounted sale shows strikethrough list price; loss is red, '
